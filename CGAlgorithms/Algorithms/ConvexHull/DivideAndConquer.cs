...
namespace CGAlgorithms.Algorithms.ConvexHull
{
    ...
    public class DivideHelper
    {
        ...
        public List<Point> Divide(List<Point> divisiblePoints)
        {
            ...
            int middleIndex = divisiblePoints.Count / 2;
            List<Point> leftPoints = divisiblePoints.Take(middleIndex).ToList();
            List<Point> rightPoints = divisiblePoints.Skip(middleIndex).ToList();

            // Recursively divide and store the results in separate variables.
            List<Point> dividedLeft = Divide(leftPoints);
            List<Point> dividedRight = Divide(rightPoints);

            // Combine the convex hulls of the left and right halves.
            return new CombineHelper().Combine(dividedLeft, dividedRight);
        }
